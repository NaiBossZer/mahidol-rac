...
// Constants
// ============================================================
const HOST_INFO: Record<
  HostTree,
  { name: string; icon: React.ElementType; desc: string; baseYield: number; basePurity: number }
> = {
  jamjuree: {
    name: "ต้นจามจุรี",
    icon: TreeDeciduous,
...
const SEASON_INFO: Record<
  SeasonCycle,
  { name: string; icon: React.ElementType; desc: string; yieldMul: number; purityAdj: number }
> = {
  sai: {
...
function calcField(
  ...
  const yieldKg =
    host.baseYield * seasonCfg.yieldMul * tempFactor * (0.6 + 0.4 * pestControlFactor);
  let purity =
    host.basePurity + seasonCfg.purityAdj + biocontrol * 0.15 - (100 - biocontrol) * 0.05;
...
function calcBCGScore(
  ...
  const total = Math.round(
    Math.max(0, Math.min(100, yieldScore + purityScore + biocontrolScore + chemistryScore)),
  );
...
const FlowPath: React.FC<{ d: string; active: boolean; color: string }> = ({
  d,
  active,
  color,
}) => (
...
const SobprabLacLabGame: React.FC = () => {
  ...
  const field = useMemo(
    () => calcField(hostTree, season, temperature, biocontrol),
    [hostTree, season, temperature, biocontrol],
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-2">
            ...
            <h1
              ...
              style={{ color: "var(--rac-brand-blue-light)" }}
            >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-sm font-medium text-slate-600 flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4" style={{ color: "#16A34A" }} /> การควบคุมแตนเบียน
                  (Biological Control)
                </label>
              ...
              <input
                ...
                step={5}
                value={biocontrol}
                onChange={(e) => setBiocontrol(Number(e.target.value))}
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <StatBox
              icon={Package}
              label="ผลผลิตครั่งดิบ (Sticklac)"
              value={`${field.yieldKg} kg`}
              color="#9E2A2B"
            />
            <StatBox
              icon={CheckCircle2}
              label="เกรดความบริสุทธิ์"
              value={`${field.purity}%`}
              color="#16A34A"
            />
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-lg text-slate-800">
              2. ห้องปฏิบัติการสี Laccaic Acid &amp; pH Chemistry
            </h2>
          </div>
                ...
                <div className="flex flex-col gap-3">
                  {(Object.keys(MORDANT_INFO) as MordantType[]).map((key) => (
                    <SelectCard
                      ...
                      color="var(--rac-brand-blue-light)"
            ...
            <div className="flex flex-col items-center justify-center gap-3">
              ...
              <div className="flex gap-4 items-center">
                <div className="flex flex-col items-center gap-1">
                  <div
                    className="w-24 h-32 rounded-lg border border-slate-300 bg-[#F5F0E8]"
                    style={{
                      backgroundImage:
                ...
                <div className="flex flex-col items-center gap-1">
                  <motion.div
                    ...
                    style={{
                      backgroundImage:
                        "repeating-linear-gradient(45deg, rgba(0,0,0,0.08) 0, rgba(0,0,0,0.08) 2px, transparent 2px, transparent 6px)",
                    }}
                  />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Factory className="w-5 h-5" style={{ color: "var(--rac-brand-blue-light)" }} />
              <h2 className="font-semibold text-lg text-slate-800">
                3. กระบวนการแปรรูปครั่งสบปราบ
              </h2>
            </div>
            <button
              ...
              style={{
                borderColor: "var(--rac-brand-blue-light)",
                color: processRunning ? "white" : "var(--rac-brand-blue-light)",
                backgroundColor: processRunning ? "var(--rac-brand-blue-light)" : "white",
              }}
            >
          ...
          <div className="w-full bg-slate-50 rounded-2xl border border-slate-200 p-3 overflow-x-auto">
            <svg viewBox="0 0 960 320" className="w-full min-w-[880px] h-80">
              ...
              <FlowNode
                ...
                h={90}
                icon={Droplets}
                label="สกัดเรซิน Shellac + Lac Wax"
                color="#F2A900"
              />
              <FlowNode
                x={730}
          ...
          <div className="flex flex-wrap gap-4 justify-center text-xs text-slate-500">
            <span className="flex items-center gap-1">
              <Apple className="w-4 h-4 text-red-500" /> เคลือบผิวผลไม้
            </span>
            <span className="flex items-center gap-1">
              <Pill className="w-4 h-4 text-sky-500" /> เคลือบเม็ดยา
            </span>
            <span className="flex items-center gap-1">
              <Hammer className="w-4 h-4 text-amber-700" /> เคลือบงานไม้
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-lg text-slate-800">
              4. Sobprab Science Master Score &amp; BCG Badge
            </h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-3">
              ...
              <p className="text-sm text-slate-500">
                สถานะ: <span className="font-semibold text-slate-700">{bcg.tier}</span>
              </p>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-600">
                        RAC Sobprab Eco-Innovator — {bcg.tier}
                      </p>
                    </div>
                  </motion.div>
                )}
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-2">
              <p className="text-sm font-semibold text-slate-700 flex items-center gap-1.5">
                ...
                ประโยชน์ทางเคมีและเศรษฐกิจชุมชนสบปราบ
              </p>
              <ul className="space-y-1.5 text-xs text-slate-600">
                <li className="flex items-start gap-2">
                  <Leaf className="w-3.5 h-3.5 text-green-600 mt-0.5 shrink-0" />{" "}
                  ครั่งเป็นทรัพยากรชีวภาพสร้างรายได้เสริมให้เกษตรกรสบปราบ
                </li>
                <li className="flex items-start gap-2">
                  <Palette className="w-3.5 h-3.5 text-pink-500 mt-0.5 shrink-0" /> Laccaic Acid
                  ใช้เป็นสีผสมอาหารธรรมชาติ ปลอดภัยกว่าสีสังเคราะห์
                </li>
                <li className="flex items-start gap-2">
                  <Pill className="w-3.5 h-3.5 text-sky-500 mt-0.5 shrink-0" /> Shellac
                  ใช้เคลือบผลไม้และเม็ดยา ช่วยยืดอายุการเก็บรักษา
                </li>
                <li className="flex items-start gap-2">
                  <Hammer className="w-3.5 h-3.5 text-amber-700 mt-0.5 shrink-0" /> Lac Wax
                  ใช้ในอุตสาหกรรมขัดเงาไม้และเครื่องหนัง
                </li>
                <li className="flex items-start gap-2">
                  <Star className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" /> โมเดล BCG
                  หมุนเวียนทรัพยากรท้องถิ่น สร้างมูลค่าเพิ่ม ลดของเสีย
                </li>
              </ul>
            </div>
          </div>
