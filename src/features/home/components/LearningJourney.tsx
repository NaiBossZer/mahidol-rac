...
export function LearningJourney() {
  ...
  const languageSuffix = i18n.language === "en" ? "en" : "th";
        ...
        <div className="relative mt-8 sm:mt-10">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {journey.map((item, index) => (
              <Link key={item.no} to={item.href} aria-label={t("learningJourney.ariaLabel", { title: t(`learningJourney.${item.key}.title`) })} className="group relative min-h-[310px] min-w-0 overflow-hidden rounded-3xl border border-[#6d4a35]/15 bg-[#2f241d] shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8f3028] focus-visible:ring-offset-2">
                ...
                <div className="relative z-10 flex min-h-[310px] flex-col p-5 sm:p-6">
                  ...
                  <h3 className="mt-6 w-fit max-w-full rounded-xl border border-white/15 bg-black/40 px-3 py-2 text-lg font-bold leading-snug text-white shadow-sm backdrop-blur-[2px] sm:text-xl">{t(`learningJourney.${item.key}.title`)}</h3>
                  <p className="mt-auto pt-5 text-sm leading-7 text-white/85">{t(`learningJourney.${item.key}.text`)}</p>
