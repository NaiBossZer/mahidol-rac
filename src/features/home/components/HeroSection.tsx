...
export function HeroSection({ currentSlide, onPrevious, onNext, onSelectSlide, onScrollToSection, isPaused, onTogglePause }: HeroSectionProps) {
  ...
  return (
    <section id="hero" aria-label="ศูนย์เรียนรู้ครั่งมหิดล ลำปาง" className="relative min-h-[560px] w-full overflow-hidden bg-rac-blue sm:min-h-[600px] lg:min-h-[680px]">
          ...
          <div className="relative z-20 mx-auto flex h-full max-w-6xl items-center px-4 pb-24 pt-24 sm:px-10 sm:pb-20 lg:px-12">
            <div className="max-w-3xl text-left text-white">
              <div className="mb-5 flex flex-wrap items-center gap-3">
                <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-white/20 bg-black/20 px-3 py-2 text-[10px] font-semibold tracking-[0.12em] text-rac-gold backdrop-blur-md sm:px-4 sm:text-[11px] sm:tracking-[0.14em]"><span className="h-2 w-2 shrink-0 rounded-full bg-rac-gold" /><span className="truncate">{item.badge}</span></span>
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/45 to-transparent px-4 pb-4 pt-16 sm:px-10 sm:pb-5"><div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4"><div className="flex items-center gap-2" role="tablist" aria-label="เลือกสไลด์">{HERO_SLIDES.map((item, idx) => <button key={item.id} type="button" role="tab" aria-selected={currentSlide === idx} aria-label={`ไปสไลด์ ${idx + 1}: ${item.title}`} onClick={() => onSelectSlide(idx)} className={`h-2 rounded-full transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold focus-visible:ring-offset-2 ${currentSlide === idx ? "w-10 bg-rac-gold" : "w-2 bg-white/55 hover:bg-white"}`} />)}</div><div className="flex items-center gap-2 self-end"><button type="button" onClick={onTogglePause} aria-label={isPaused ? "เล่นสไลด์อัตโนมัติ" : "หยุดสไลด์อัตโนมัติ"} className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/25 text-white backdrop-blur-sm transition hover:bg-black/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold">{isPaused ? <Play size={16} /> : <Pause size={16} />}</button><button type="button" onClick={onPrevious} aria-label="สไลด์ก่อนหน้า" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/25 text-white backdrop-blur-sm transition hover:bg-black/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold"><ChevronLeft size={20} /></button><button type="button" onClick={onNext} aria-label="สไลด์ถัดไป" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/25 text-white backdrop-blur-sm transition hover:bg-black/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold"><ChevronRight size={20} /></button></div></div></div>
      <button type="button" onClick={() => onScrollToSection("what-is-lac")} aria-label="เลื่อนลงเพื่อดูครั่งคืออะไร" className="absolute bottom-20 left-1/2 z-30 hidden -translate-x-1/2 items-center gap-2 rounded-full border border-white/15 bg-black/20 px-4 py-2 text-[11px] font-medium text-white/80 backdrop-blur-sm transition hover:bg-black/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold sm:flex"><ArrowDown size={14} />สำรวจครั่งคืออะไร</button>
      <span className="sr-only" aria-live="polite">สไลด์ปัจจุบัน: {slide.title}</span>
    </section>
  );
}
