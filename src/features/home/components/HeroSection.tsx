...
export function HeroSection({ currentSlide, onPrevious, onNext, onSelectSlide, onScrollToSection, isPaused, onTogglePause }: HeroSectionProps) {
  ...
  return (
    <section id="hero" aria-label="ศูนย์เรียนรู้ครั่งมหิดล ลำปาง" className="relative min-h-[560px] w-full overflow-hidden bg-rac-blue sm:min-h-[600px] lg:min-h-[680px]">
      ...
      <button type="button" onClick={() => onScrollToSection("cards-section")} aria-label="เลื่อนลงเพื่อดูองค์ความรู้" className="absolute bottom-20 left-1/2 z-30 hidden -translate-x-1/2 items-center gap-2 rounded-full border border-white/15 bg-black/20 px-4 py-2 text-[11px] font-medium text-white/80 backdrop-blur-sm transition hover:bg-black/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold sm:flex"><ArrowDown size={14} />สำรวจองค์ความรู้</button>
