...
export function HeroSection({
  ...
  return (
    ...
    >
              ...
              <div className="max-w-3xl text-left text-white">
                <div className="mb-5 flex flex-wrap items-center gap-3">
                  <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-white/20 bg-black/20 px-3 py-2 text-[10px] font-semibold tracking-[0.12em] text-rac-gold backdrop-blur-md sm:px-4 sm:text-[11px] sm:tracking-[0.14em]">
                    <span className="h-2 w-2 shrink-0 rounded-full bg-rac-gold" />
                    <span className="truncate">{t(item.badgeKey)}</span>
                  </span>
                ...
                <h1 className="rac-display max-w-3xl text-3xl font-bold leading-[1.18] tracking-tight drop-shadow-md sm:text-5xl lg:text-6xl">
                  {title}
                </h1>
                <p className="mt-4 max-w-2xl text-sm leading-6 text-white/85 drop-shadow sm:text-base sm:leading-7 lg:text-lg">
                  {t(item.subtitleKey)}
                </p>
                <div className="mt-8 flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center">
                  ...
                  >
                    <span>{t(item.buttonTextKey)}</span>
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
          ...
          >
            {HERO_SLIDES.map((item, idx) => (
              <button
                ...
                aria-label={t("hero.goToSlide", { index: idx + 1, title: t(item.titleKey) })}
                onClick={() => onSelectSlide(idx)}
                className={`h-2 rounded-full transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-rac-gold focus-visible:ring-offset-2 ${currentSlide === idx ? "w-10 bg-rac-gold" : "w-2 bg-white/55 hover:bg-white"}`}
              />
            ))}
          </div>
          <div className="flex items-center gap-2 self-end">
            <button
              ...
              aria-label={isPaused ? t("nav.playAutoSlides") : t("nav.pauseAutoSlides")}
      ...
      <button
        type="button"
        onClick={() => onScrollToSection("what-is-lac")}
        aria-label={t("hero.exploreWhatIsLac")}
      ...
      >
        <ArrowDown size={14} />
        {t("hero.exploreWhatIsLac")}
      </button>
      <span className="sr-only" aria-live="polite">
        {t("hero.currentSlide", { title: t(slide.titleKey) })}
      </span>
    </section>
  );
}
