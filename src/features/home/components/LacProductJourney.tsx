  ...
  return (
    <section id="lac-product" className="scroll-mt-24 bg-[#4b2b24] px-4 py-14 text-white sm:py-20" aria-labelledby="lac-product-title">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8">
          <p className="whitespace-nowrap text-[9px] font-semibold tracking-[0.12em] text-amber-300 sm:text-[10px] sm:tracking-[0.2em]">07 — LAC → PRODUCT</p>
          ...
          {products.map((product) => (
            <article key={product.no} className="rounded-2xl border border-white/10 bg-white/[0.06] p-5 transition hover:bg-white/[0.09] sm:p-6">
              <div className="flex items-center justify-between">
                <span className="font-mono text-xs font-bold tracking-[0.16em] text-amber-300">{product.no}</span>
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.05] p-5 sm:mt-10 sm:p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {uses.map((use) => <span key={use} className="rounded-full border border-white/10 bg-white/[0.07] px-3 py-1.5 text-xs text-white/80">{use}</span>)}
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-2 text-sm text-white/65 sm:mt-10 sm:gap-3" aria-label="เส้นทางการเรียนรู้จากครั่งสู่ผลิตภัณฑ์">
          <span className="rounded-full border border-white/10 px-3 py-2 sm:px-4">ครั่งดิบ</span><span aria-hidden="true">→</span>
          <span className="rounded-full border border-white/10 px-3 py-2 sm:px-4">การแปรรูป</span><span aria-hidden="true">→</span>
