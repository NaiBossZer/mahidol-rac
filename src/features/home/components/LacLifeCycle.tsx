...
export function LacLifeCycle() {
  return (
    <section id="life-cycle" className="scroll-mt-24 bg-[#4b2b24] px-4 py-14 text-white sm:py-20" aria-labelledby="life-cycle-title">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-8 lg:grid-cols-2 lg:items-center lg:gap-16">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-7 text-white/75 sm:text-base">สำรวจแต่ละช่วงของวงจรชีวิต ตั้งแต่ไข่จนถึงตัวเต็มวัย ผ่านลำดับการเรียนรู้ที่เชื่อมโยงกับการเพาะเลี้ยงครั่ง</p>
            <button type="button" className="mt-7 rounded-lg bg-[#8f3028] px-6 py-3 text-sm font-semibold text-white transition hover:opacity-90">สำรวจวงจรชีวิต →</button>
          </div>
          <div className="relative flex flex-col gap-3">
            ...
            {stages.map((stage) => (
              <article key={stage.no} className="relative flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.06] p-4 sm:p-5">
                ...
                <div className="min-w-0"><p className="font-mono text-[9px] tracking-[0.12em] text-amber-300">{stage.no}</p><h3 className="text-base font-bold sm:text-lg">{stage.title}</h3><p className="mt-1 text-xs leading-5 text-white/70 sm:text-sm sm:leading-6">{stage.text}</p></div>
              </article>
            ))}
          </div>
        </div>
