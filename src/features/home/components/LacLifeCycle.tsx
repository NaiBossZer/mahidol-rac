    ...
    <section id="life-cycle" className="scroll-mt-24 bg-rac-blue-deep px-4 py-14 text-white sm:py-20" aria-labelledby="life-cycle-title">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-8 lg:grid-cols-2 lg:items-center lg:gap-16">
          <div>
            <p className="whitespace-nowrap text-[9px] font-semibold tracking-[0.12em] text-amber-300 sm:text-[10px] sm:tracking-[0.2em]">03 — LIFE CYCLE</p>
            <h2 id="life-cycle-title" className="mt-3 text-3xl font-bold leading-tight tracking-tight sm:text-4xl">
              วงจรชีวิต<br /><span className="text-amber-300">ของครั่ง</span>
            </h2>
            ...
            {stages.map((stage) => (
              <article key={stage.no} className="relative flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.06] p-4 sm:p-5">
                ...
                <div className="min-w-0">
                  <p className="font-mono text-[9px] tracking-[0.12em] text-amber-300">{stage.no}</p>
                  <h3 className="text-base font-bold sm:text-lg">{stage.title}</h3>
                  <p className="mt-1 text-xs leading-5 text-slate-300 sm:text-sm sm:leading-6">{stage.text}</p>
                </div>
              </article>
            ))}
          </div>
