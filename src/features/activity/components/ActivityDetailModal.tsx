    ...
    >
      ...
      <div className="grid max-h-[92vh] w-full max-w-6xl overflow-auto rounded-3xl bg-white shadow-2xl lg:grid-cols-[1.35fr_.65fr]">
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
              {images.map((src, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => setIndex(i)}
                  className={`h-16 w-24 shrink-0 overflow-hidden rounded-lg border-2 ${i === index ? "border-rac-lac" : "border-transparent"}`}
                >
        ...
        <div className="p-5 sm:p-7">
          <p className="text-xs font-bold text-rac-lac">
          ...
          <div className="mt-6 space-y-5 text-sm text-slate-600">
            ...
            <Detail
              icon={<ListChecks />}
              title="กิจกรรมสำคัญ"
              value={activity.keyActivities?.join(" • ") || "-"}
            />
...
function Detail({ icon, title, value }: { icon: ReactNode; title: string; value: string }) {
  return (
    <section>
      <div className="flex items-center gap-2 font-bold text-slate-800">
        <span className="text-rac-lac">{icon}</span>
        {title}
      </div>
