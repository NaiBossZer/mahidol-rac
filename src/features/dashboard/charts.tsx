...
export function StackedRatingBars({
  ...
  if (!data.length) return <Empty />;
  const keys = Array.from(new Set(data.flatMap((d) => Object.keys(d.buckets)))).sort(
...
const CHANNEL_RULES: { key: string; re: RegExp; color: string }[] = [
  ...
  { key: "Email", re: /e-?mail|อีเมล|จดหมาย|หนังสือเชิญ|หนังสือ/i, color: "var(--color-chart-3)" },
  {
    ...
    re: /instagram|\big\b|tiktok|twitter|\bx\b/i,
    color: "var(--color-chart-4)",
  },
  { key: "YouTube", re: /youtube|ยูทู/i, color: "var(--color-chart-5)" },
  {
    key: "เพื่อน / คนรู้จัก",
    re: /เพื่อน|คนรู้จัก|บอกต่อ|ปากต่อปาก|อาจารย์|รุ่นพี่/i,
    color: "var(--color-chart-6)",
  },
  {
    key: "หน่วยงาน / ต้นสังกัด",
    re: /หน่วยงาน|ต้นสังกัด|คณะ|ผู้บังคับบัญชา|ประกาศ/i,
    color: "var(--color-gold)",
  },
  {
    key: "โปสเตอร์ / ป้าย",
    re: /โปสเตอร์|ป้าย|แผ่นพับ|ไวนิล|poster/i,
