...
const mapSurveyRow = (row: Record<string, unknown>): SurveyResponse => ({
  ...row,
  p4_communityResource: (row.p4_community_resource ?? row.p4_communityResource) as
    number | undefined,
  p4_futureReturn: (row.p4_future_return ?? row.p4_futureReturn) as number | undefined,
...
export function useDashboardData() {
  ...
  const [selectedActivity, setSelectedActivity] = useState(
    () => new URLSearchParams(window.location.search).get("activity") || "ALL",
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [responseResult, activityResult] = await Promise.all([
        ...
        supabase
          .from("activities")
          .select("id,title,activity_date,category,featured_image,status")
      ...
      setData(
        (responseResult.data ?? []).map((row) => mapSurveyRow(row as Record<string, unknown>)),
      );
      ...
      setErrorMsg(
        "ไม่สามารถดึงข้อมูลจาก Supabase ได้ กรุณาตรวจสอบสิทธิ์ Admin และกด Refresh อีกครั้ง",
      );
      setData([]);
      setActivities([]);
    } finally {
  ...
  const selectedActivityInfo =
    selectedActivity === "ALL" ? null : activityMap.get(selectedActivity) || null;
  ...
  const filteredData = useMemo(
    () =>
      data.filter((item) => {
        if (selectedActivity !== "ALL" && String(item.activity_id || "") !== selectedActivity)
          return false;
        const date = item.timestamp ? new Date(item.timestamp) : null;
  ...
  const categoryGroupedScores = useMemo(() => {
    ...
    itemScores.forEach((item) => {
      (groups[item.category] ??= { category: item.category, items: [] }).items.push(item);
    });
