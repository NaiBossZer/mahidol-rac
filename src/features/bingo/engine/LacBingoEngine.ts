...
export function lacBingoReducer(state: LacBingoState, action: LacBingoAction): LacBingoState {
  switch (action.type) {
    ...
    case "submit_answer": {
      if (!state.activeQuestion || state.selectedOption === null || state.isAnswerChecked) return state;

      const isCorrect = state.selectedOption === state.activeQuestion.correctIndex;
    ...
    case "tick":
      return { ...state, secondsElapsed: state.secondsElapsed + Math.max(0, action.seconds ?? 1) };

    case "set_team_name":
      return { ...state, teamName: action.teamName };

    case "save_score":
      if (state.isSavedToLeaderboard) return state;
      return {
        ...state,
    ...
    case "hide_bingo_banner":
      return {
        ...state,
        showBingoBanner: false,
        phase: state.isFullBingo ? "victory" : state.isAnswerChecked ? "answer-result" : "ready",
      };
