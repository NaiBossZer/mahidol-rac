...
export function useLacBingoEngine() {
  const [state, dispatch] = useReducer(lacBingoReducer, undefined, () =>
  ...
  const liveTimeBonus =
    state.isGameOver && BINGO_DIFFICULTY_RULES[state.difficulty].hasTimeLimit
      ? state.timeBonus
      : BINGO_DIFFICULTY_RULES[state.difficulty].hasTimeLimit
  ...
  const drawQuestionForTile = useCallback(
    (tileId: string) => {
      if (state.activeQuestion || state.isGameOver) return null;
  ...
  const setTeamName = useCallback(
    (teamName: string) => dispatch({ type: "set_team_name", teamName }),
    [],
  );
  const inspectTile = useCallback(
    (tile: BingoTile | null) => dispatch({ type: "inspect_tile", tile }),
    [],
  );
  ...
  const setHostMode = useCallback(
    (mode: "player" | "screen") => dispatch({ type: "set_host_mode", mode }),
    [],
  );
  const setDifficulty = useCallback(
    (difficulty: BingoDifficulty) => dispatch({ type: "set_difficulty", difficulty }),
    [],
  );
