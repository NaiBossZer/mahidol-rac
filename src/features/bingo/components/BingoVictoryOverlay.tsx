...
function getBadge(
  ...
  if (lines >= 4 && accuracy >= 80)
    return { icon: "🏆", name: "Lac Expert", description: "พิชิตเป้าหมาย 4 สาย" };
  if (lines >= 2)
    return { icon: "⭐", name: "Lac Explorer", description: "ค้นพบเส้นทางความรู้แล้ว" };
...
export const BingoVictoryOverlay: React.FC<BingoVictoryOverlayProps> = ({
  ...
  const message = isFullBingo
    ? "ปลดล็อกครบทั้ง 16 ช่องและผ่านภารกิจบิงโก"
    : "ครบ 5 นาทีแล้ว สรุปผลการเรียนรู้และคะแนนรอบนี้";
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="bingo-victory-title"
    >
      <div className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border-2 border-amber-300/70 bg-slate-900 text-white shadow-2xl">
        ...
        <div className="bg-gradient-to-br from-slate-900 via-rac-blue to-rac-lac px-6 pb-7 pt-8 text-center sm:px-8">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl border border-amber-300/40 bg-white/10 shadow-lg">
          ...
          <h2 id="bingo-victory-title" className="mt-1 text-2xl font-bold sm:text-3xl">
            {title}
          </h2>
          <p className="mt-2 text-sm text-white/75">
            {message} • {teamName.trim() || "ทีมนิรนาม"}
          </p>
        </div>
        ...
        <div className="space-y-4 p-5 sm:p-7">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-3">
            <div
              className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-300/15 text-3xl"
              aria-hidden="true"
            >
              {badge.icon}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-xl border border-amber-300/20 bg-amber-300/10 p-3">
              <p className="text-[10px] text-amber-200/70">Time Bonus</p>
              <p className="mt-0.5 text-lg font-black text-amber-300">+{timeBonus}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              <p className="text-[10px] text-slate-400">สถานะ</p>
              <p className="mt-0.5 flex items-center gap-1 text-sm font-bold text-emerald-300">
                {isTimeUp ? <TimerOff className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}{" "}
                {isTimeUp ? "หมดเวลา" : "BINGO!"}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4 text-sm text-emerald-100">
            ...
            <p className="mt-1 text-xs leading-relaxed text-emerald-100/75">
              คะแนนคิดจากตอบถูก × 10, หักตอบผิด × 5, โบนัสสายบิงโก × 50 และ Time Bonus
              ตามเวลาที่เหลือ
            </p>
          </div>
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button
              ref={replayRef}
              onClick={onReplay}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-amber-400"
            >
            ...
            <button
              onClick={onSave}
              disabled={isSaved}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-bold text-slate-950 shadow-md transition hover:bg-amber-300 disabled:cursor-default disabled:bg-emerald-500 disabled:text-white focus:outline-none focus:ring-2 focus:ring-amber-300"
            >
...
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-center">
      <div className="mx-auto flex h-7 w-7 items-center justify-center rounded-lg bg-white/10 text-amber-300">
        {icon}
      </div>
