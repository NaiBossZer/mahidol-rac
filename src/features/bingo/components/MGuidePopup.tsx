...
export const MGuidePopup: React.FC<MGuidePopupProps> = ({ isOpen, onClose, question, onAnswer, selectedOption, isAnswerChecked, isCorrect, onNextQuestion, emotion, soundEnabled, difficulty }) => {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/80 p-3 backdrop-blur-md sm:p-4" role="dialog" aria-modal="true" aria-labelledby="mguide-modal-title" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div ref={modalRef} className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border-2 border-amber-300/30 bg-slate-900 text-white shadow-2xl">
        <div className="relative flex shrink-0 items-center justify-between overflow-hidden bg-gradient-to-r from-slate-900 via-rac-blue to-rac-lac p-4 text-white sm:p-5">
          ...
          <div className="flex items-center gap-3"><div className="relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl border-2 border-white/20 bg-white/10 p-1 shadow-inner sm:h-14 sm:w-14"><img src={`/assets/characters/prof-mahidol/${emotion === "excited" ? "excited" : emotion === "happy" ? "happy" : emotion === "thinking" ? "thinking" : emotion === "concerned" ? "concerned" : "idle"}.svg`} alt="พี่ M-Guide" className="h-full w-full object-contain transition-transform hover:scale-110" onError={(e) => { e.currentTarget.style.display = "none"; }} /><span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white bg-green-400" /></div><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-amber-400 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-950 sm:text-xs">BINGO HOST</span><span className="font-mono text-xs text-white/60">การ์ดคำถาม #{question.id}</span><span className="rounded-full border border-cyan-200/30 bg-cyan-200/10 px-2 py-0.5 text-[10px] font-bold text-cyan-100">ระดับ{difficultyRules.label}</span></div><h3 id="mguide-modal-title" className="text-base font-bold leading-tight text-white sm:text-lg">พี่ M-Guide (ผู้ช่วยศูนย์เรียนรู้ครั่ง)</h3></div></div>
        ...
        <div className="space-y-4 overflow-y-auto p-4 sm:p-6">
          ...
          {!difficultyRules.showHint && !isAnswerChecked && <div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/5 p-3 text-xs text-cyan-100"><span className="font-bold">โหมดท้าทาย:</span> รอบนี้ไม่มีคำใบ้ · ตอบถูกได้ +{difficultyRules.correctScore} คะแนน</div>}
