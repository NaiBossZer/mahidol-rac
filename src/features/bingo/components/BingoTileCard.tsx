    ...
    >
      {tile.isHighlighted && (
        <motion.div
          ...
          className="pointer-events-none absolute inset-0 rounded-2xl bg-cyan-200/20"
          aria-hidden="true"
        />
      )}
      ...
      <div className="absolute right-3 top-3 z-10 sm:right-4 sm:top-4">
        <div
          className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${tile.isMarked ? "bg-slate-950/10 text-slate-950" : "bg-white/10 text-white"}`}
          aria-hidden="true"
        >
          {tile.isMarked ? (
      ...
      <div className="flex h-full w-full items-center justify-center px-1 sm:px-2">
        <h3
          className={`w-full break-words text-center text-[clamp(1.25rem,2.25vw,1.9rem)] font-black leading-[1.08] tracking-tight text-balance ${tile.isMarked ? "text-slate-950" : "text-white"}`}
        >
          {tile.keyword}
        </h3>
      </div>
      ...
      {isInWinningLine && (
        <div
          aria-hidden="true"
          className="absolute -right-1 -top-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-slate-950/20 bg-white text-xs font-black text-slate-950 shadow"
        >
          ★
        </div>
      )}
