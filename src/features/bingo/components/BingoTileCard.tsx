...
import { Check, TreeDeciduous, Package, Sparkles, FlaskConical, Pill, MapPin, Clock, Bug, Building2, Leaf, Palette, Layers as LayersIcon, LockKeyhole } from "lucide-react";
...
export const BingoTileCard: React.FC<BingoTileCardProps> = ({ tile, index, isInWinningLine, onClick }) => {
  ...
  return (
    <motion.button
      ...
      className={`relative min-h-0 h-full w-full overflow-hidden rounded-2xl border-2 p-3 text-left select-none focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-950 sm:p-4 ${
        tile.isMarked
          ? isInWinningLine
    ...
    >
      ...
      <div className="relative flex h-full flex-col justify-center">
        ...
        <div className="my-2 min-h-0 flex-1 flex items-center">
          <h3 className={`w-full line-clamp-3 text-[clamp(1.1rem,3.3vw,2rem)] font-black leading-tight tracking-tight ${tile.isMarked ? "text-slate-950" : "text-white"}`}>{tile.keyword}</h3>
        </div>
      </div>
