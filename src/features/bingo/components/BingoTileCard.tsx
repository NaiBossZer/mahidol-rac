...
export const BingoTileCard: React.FC<BingoTileCardProps> = ({
  ...
  return (
    <button
      ...
      className={`relative text-left p-3 sm:p-3.5 rounded-2xl border-2 transition-all duration-300 flex flex-col justify-between min-h-[110px] sm:min-h-[125px] cursor-pointer group select-none focus:outline-none focus:ring-2 focus:ring-rac-lac ${
        tile.isMarked
          ? isInWinningLine
          ...
          : "bg-white border-slate-200 hover:border-rac-lac/40 hover:bg-slate-50/60 shadow-xs"
      } ${tile.isHighlighted ? "ring-4 ring-rac-gold animate-pulse" : ""}`}
    >
      ...
      <div className="my-1.5">
        <h3
          className={`text-xs sm:text-sm font-bold leading-tight ${
            tile.isMarked ? "text-slate-900" : "text-slate-800 group-hover:text-rac-lac"
          }`}
        >
          {tile.keyword}
        </h3>
      ...
      <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1 border-t border-slate-100 w-full">
        ...
        <span className="text-[9px] font-semibold text-slate-500">
          {tile.isMarked ? (
            ...
            <span className="group-hover:text-rac-lac transition-colors">คลิกทาย ›</span>
          )}
        </span>
      </div>
      ...
      {isInWinningLine && (
        <div
          aria-hidden="true"
          className="absolute -top-1 -right-1 w-4 h-4 bg-rac-gold rounded-full border border-white flex items-center justify-center text-[8px] font-black text-slate-900 shadow"
        >
          ★
        </div>
      )}
