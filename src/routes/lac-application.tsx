...
const stories = [
  {
    ...
    insight:
      "ครั่งในมุมของเกษตรกร — จากสิ่งมีชีวิตขนาดเล็กบนต้นพิงอาศัย สู่โอกาสในการสร้างรายได้จากทรัพยากรในพื้นที่",
  },
  {
    id: "lac-water",
    no: "02",
    ...
    insight:
      "ครั่งในมุมของการเกษตร — เรื่องราวการต่อยอดองค์ความรู้ครั่งไปสู่แนวทางที่ช่วยลดต้นทุนการผลิต",
  },
] as const;
        ...
        <RacContainer className="flex min-h-[calc(100vh-60px)] flex-col px-3 py-3 sm:px-4 sm:py-4">
          <header className="shrink-0">
            <p className="font-mono text-[9px] font-semibold tracking-[0.18em] text-[#8f3328] sm:text-[10px]">
              FROM LAC TO APPLICATION
            </p>
            <div className="mt-1 flex items-end justify-between gap-4">
              <div className="min-w-0">
                <h1 className="truncate text-2xl font-bold tracking-tight sm:text-3xl">
                  จากครั่งสู่การใช้ประโยชน์
                </h1>
                <p className="mt-1 hidden text-xs text-slate-600 sm:block">
                  จากวัสดุธรรมชาติ → การต่อยอด → การใช้ประโยชน์จริง
                </p>
              </div>
              <span className="hidden rounded-full border border-[#8f3328]/15 bg-white/60 px-3 py-1.5 text-[10px] font-medium text-[#8f3328] sm:inline-flex">
                Story Room
              </span>
            </div>
          </header>
          <section className="min-h-0 flex-1 py-3" aria-label="เรื่องราวจาก ENV Mahidol">
            <div className="grid h-full min-h-0 overflow-hidden rounded-[24px] border border-[#8f3328]/15 bg-white lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.75fr)]">
              <div className="relative min-h-0 bg-black">
                ...
                />
                <div className="pointer-events-none absolute left-3 top-3 rounded-lg bg-black/45 px-2.5 py-1 text-[10px] text-white/80 backdrop-blur-sm">
                  ENV Mahidol
                </div>
              </div>
              <aside className="min-h-0 overflow-hidden bg-[#f3eadb] p-4 sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-1 text-sm font-bold text-[#8f3328]">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <span
                            className={`mt-0.5 font-mono text-[10px] font-bold ${active ? "text-white/80" : "text-[#8f3328]"}`}
                          >
                            {story.no}
                          </span>
                          <span className="min-w-0">
                            ...
                            <span
                              className={`mt-0.5 block text-[11px] leading-4 ${active ? "text-white/75" : "text-slate-500"}`}
                            >
                              {story.subtitle}
                            </span>
                          </span>
