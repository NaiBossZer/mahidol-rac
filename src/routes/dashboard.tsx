...
export function DashboardPage() {
  ...
  const selectedActivityInfo = selectedActivity === "ALL" ? null : activities.find((activity) => activity.id === selectedActivity) || null;

  useEffect(() => {
    let active = true;
    const load = async () => {
      if (!supabase) { setLoadingActivities(false); setLoadingCount(false); return; }
      const { data, error } = await supabase.from("activities").select("id,title,activity_date,status,featured_image").order("activity_date", { ascending: false });
      if (active) {
        if (error) console.error("Failed to load canonical activities", error);
  ...
  useEffect(() => {
    ...
    const loadCount = async () => {
      if (!supabase) { setResponseCount(0); setLoadingCount(false); return; }
      ...
      if (active) { setResponseCount(count ?? 0); setLoadingCount(false); }
    };
    void loadCount();
    return () => { active = false; };
  }, [selectedActivity]);
  ...
  const changeActivity = (activityId: string) => {
    ...
    if (activityId === "ALL") params.delete("activity"); else params.set("activity", activityId);
