...
const lessonPages = {
  "what-is-lac": {
    ...
    eyebrow: "ความรู้พื้นฐาน",
    sectionTitle: "จากสิ่งมีชีวิตตัวเล็กสู่ครั่ง",
...
function LessonRoom({ slug, children }: { slug: LessonSlug; children: React.ReactNode }) {
      ...
      <RacContainer>
        <section className="py-8 sm:py-12" aria-labelledby="lesson-room-title">
          <div className="mb-5 flex items-end justify-between gap-4">
            ...
            <span className="hidden rounded-full border border-[#6d4a35]/15 bg-white/60 px-3 py-1.5 text-xs text-slate-500 sm:inline-flex">Animation / Motion Graphic</span>
          </div>

          <div className="grid overflow-hidden rounded-[28px] bg-[#211914] lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.75fr)]">
            <div className="relative aspect-video min-h-[240px] bg-[#30231c]">
              <div className="absolute inset-0 grid place-items-center">
                <div className="text-center">
                  ...
                  <p className="mt-4 text-sm font-semibold text-[#f3ebdd]">พื้นที่ Animation</p>
                  <p className="mt-1 text-xs text-[#f3ebdd]/55">วิดีโอสำหรับบทเรียนนี้จะถูกเติมภายหลัง</p>
                </div>
              </div>
              <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 bg-black/35 px-4 py-3 text-xs text-white/60">
                <span>00:00</span>
            ...
            <aside className="border-t border-white/10 p-5 text-[#f3ebdd] lg:border-l lg:border-t-0 sm:p-6">
              <p className="font-mono text-[10px] font-semibold tracking-[0.16em] text-[#c58a3a]">LEARNING OBJECTIVES</p>
              <ul className="mt-4 space-y-3">
                {lesson.objectives.map((objective) => (
                  <li key={objective} className="flex gap-3 text-sm leading-6 text-[#f3ebdd]/80">
                    <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#c58a3a]" aria-hidden="true" />
                    <span>{objective}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-6 border-t border-white/10 pt-5">
                <p className="text-xs font-semibold text-[#ead9c4]">เรียนรู้ด้วยภาพเป็นหลัก</p>
                <p className="mt-2 text-xs leading-6 text-white/55">ข้อความด้านข้างทำหน้าที่เป็นตัวช่วย ไม่แย่งความสนใจจาก Animation</p>
              </div>
            </aside>
          </div>
        </section>
        ...
        <section className="pb-10 sm:pb-14">
          <RacSectionHeader eyebrow="KEY FACTS" title="สิ่งสำคัญที่ควรจำ" description="สรุปข้อมูลหลักของบทเรียนให้เห็นได้หลังดู Animation" />
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {lesson.facts.map((fact, index) => (
              <article key={fact} className="rounded-3xl border border-[#6d4a35]/15 bg-white/70 p-5 sm:p-6">
                <p className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#8f3328]">0{index + 1}</p>
                <p className="mt-3 text-sm font-semibold leading-7 text-slate-800">{fact}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="pb-12 sm:pb-16">
          <div className="rounded-3xl bg-[#4b2b24] p-6 text-[#f3ebdd] sm:p-8">
            <p className="font-mono text-[10px] font-semibold tracking-[0.16em] text-[#c58a3a]">KEY TAKEAWAY</p>
            <h2 className="mt-3 text-xl font-bold sm:text-2xl">จำภาพรวมของบทนี้</h2>
            <p className="mt-5 break-words text-base font-semibold leading-8 text-[#ead9c4] sm:text-lg">{lesson.takeaway}</p>
          </div>
        </section>

        <section className="border-t border-[#6d4a35]/10 py-8 sm:py-10">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            {previous ? (
              <Link to={previous.href} className="rounded-2xl border border-[#6d4a35]/15 bg-white/60 px-4 py-3 text-sm font-semibold hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c58a3a]">← {previous.title}</Link>
            ) : <span />}
            {next ? (
              <Link to={next.href} className="rounded-2xl bg-[#8f3328] px-5 py-3 text-sm font-semibold text-white hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c58a3a]">บทถัดไป: {next.title} →</Link>
            ) : (
              <Link to="/" className="rounded-2xl bg-[#8f3328] px-5 py-3 text-sm font-semibold text-white hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c58a3a]">กลับสู่ศูนย์การเรียนรู้ →</Link>
            )}
          </div>
        </section>
      </RacContainer>
...
function LifeCycleContent() {
  ...
  return (
    <section className="pb-10 sm:pb-14">
      <RacSectionHeader eyebrow="LIFE CYCLE MAP" title="4 ระยะหลัก" description="ดูวงจรชีวิตเป็นลำดับ เพื่อเชื่อมโยงกับการเพาะเลี้ยงครั่ง" />
      <div className="mt-6 grid gap-3 lg:grid-cols-4">
        {stages.map(([icon, title, text], index) => (
          <article key={title} className="relative rounded-3xl border border-[#6d4a35]/15 bg-white/70 p-5">
            <p className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#8f3328]">0{index + 1}</p>
            <div className="mt-4 grid h-12 w-12 place-items-center rounded-full border border-[#c58a3a]/30 bg-[#faf7f0] text-2xl" aria-hidden="true">{icon}</div>
...
function HabitatContent() {
  ...
  return (
    <section className="pb-10 sm:pb-14">
      <RacSectionHeader eyebrow="RELATION MAP" title="ความสัมพันธ์ในระบบ" description="อ่านจากสิ่งมีชีวิต ไปสู่พืช น้ำเลี้ยง และสภาพแวดล้อม" />
        ...
        {relations.map(([icon, title, text], index) => (
          <article key={title} className="rounded-3xl border border-[#6d4a35]/15 bg-white/70 p-5 sm:p-6">
            <div className="flex items-start gap-4">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#faf7f0] text-2xl" aria-hidden="true">{icon}</div>
              <div>
                <p className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#8f3328]">0{index + 1}</p>
...
function HostPlantsContent() {
  ...
  return (
    <section className="pb-10 sm:pb-14">
      <RacSectionHeader eyebrow="HOST PLANT LIBRARY" title="ตัวอย่างต้นพิงอาศัย" description="รายชื่อที่ปรากฏในองค์ความรู้ของ repository" />
...
function FarmingContent() {
  ...
  return (
    <section className="pb-10 sm:pb-14">
      <RacSectionHeader eyebrow="FARMING FLOW" title="ลำดับการเพาะเลี้ยง" description="มองการเพาะเลี้ยงเป็นขั้นตอนต่อเนื่องจากความรู้สู่การปฏิบัติ" />
...
export function LacPage() {
  ...
  if (slug === "what-is-lac") {
    return (
      <LessonRoom slug="what-is-lac">
        <section className="pb-10 sm:pb-14">
          <RacSectionHeader eyebrow="VISUAL EXPLANATION" title="ครั่งเกิดขึ้นอย่างไร?" description="อ่านตามลำดับเดียวกับที่ Animation ควรเล่า" />
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {["🪲 แมลงครั่ง", "🌳 ต้นพิงอาศัย", "💧 ดูดน้ำเลี้ยง", "🔴 Resin / ชัน", "✨ ครั่ง"].map((item, index) => (
              <article key={item} className="rounded-3xl border border-[#6d4a35]/15 bg-white/70 p-5 text-center">
                <p className="font-mono text-[10px] font-semibold tracking-[0.14em] text-[#8f3328]">0{index + 1}</p>
                <p className="mt-4 text-sm font-bold leading-7">{item}</p>
              </article>
            ))}
          </div>
          <div className="mt-5 max-w-4xl text-sm leading-8 text-slate-700 sm:text-[15px]">
  ...
  if (slug === "life-cycle") {
    return <LessonRoom slug="life-cycle"><LifeCycleContent /></LessonRoom>;
  }
  if (slug === "habitat") {
    return <LessonRoom slug="habitat"><HabitatContent /></LessonRoom>;
  }
  if (slug === "host-plants") {
    return <LessonRoom slug="host-plants"><HostPlantsContent /></LessonRoom>;
  }
  if (slug === "lac-farming") {
    return <LessonRoom slug="lac-farming"><FarmingContent /></LessonRoom>;
  }
