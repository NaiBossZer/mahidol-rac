...
const lessonPages = {
  ...
  "life-cycle": {
    ...
    facts: [
      "ตัวผู้ประมาณ 55–60 วัน",
      "ตัวเมียให้ตัวอ่อนประมาณ 200–500 ตัว",
      "วงจรชีวิตเชื่อมโยงกับการสร้างเรซิน",
    ],
  ...
  habitat: {
    ...
    facts: [
      ...
      "ต้นพิงอาศัยเป็นแหล่งน้ำเลี้ยง",
      "สภาพแวดล้อมเกี่ยวข้องกับการเจริญเติบโต",
    ],
      ...
      {journey.map((item) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-0.5 truncate text-[10px] leading-4 text-slate-700 sm:text-xs">
              {item.title}
            </p>
          </Link>
        );
        ...
        <header className="shrink-0 pt-3 sm:pt-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="font-mono text-[9px] font-semibold tracking-[0.18em] text-[#8f3328] sm:text-[10px]">
                {lesson.no} — {lesson.code}
              </p>
              <h1 className="mt-1 truncate text-2xl font-bold tracking-tight sm:text-3xl">
                {lesson.title}
              </h1>
              <p className="mt-1 hidden max-w-3xl truncate text-xs text-slate-600 sm:block">
                {lesson.intro}
              </p>
            </div>
            <span className="hidden shrink-0 rounded-full border border-[#8f3328]/15 bg-white/60 px-3 py-1.5 text-[10px] text-[#8f3328] sm:inline-flex">
              Interactive Lesson
            </span>
          </div>
          <div className="mt-3">
        ...
        <section className="min-h-0 flex-1 py-2.5 sm:py-3" aria-labelledby="lesson-room-title">
          <div className="grid h-full min-h-0 overflow-hidden rounded-[24px] bg-[#8f3328] lg:grid-cols-[minmax(0,1.55fr)_minmax(260px,0.75fr)]">
              ...
              {lesson.video ? (
                ...
                <div className="absolute inset-0 grid place-items-center bg-[#8f3328]/20">
                  <div className="text-center">
                    <div
                      className="mx-auto grid h-14 w-14 place-items-center rounded-full border border-[#f3eadb]/45 bg-[#f3eadb]/10 text-xl text-[#f3eadb] sm:h-16 sm:w-16 sm:text-2xl"
                      aria-hidden="true"
                    >
                      ▶
                    </div>
                    <p id="lesson-room-title" className="mt-3 text-sm font-semibold text-[#f3eadb]">
                      Animation Room
                    </p>
                    <p className="mt-1 text-[11px] text-[#f3eadb]/65">
                      พื้นที่สำหรับ Animation / Motion Graphic
                    </p>
                  </div>
                </div>
              )}
              {lesson.video && (
                <div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-white/15 bg-black/45 px-2.5 py-1 text-[10px] text-white/80 backdrop-blur-sm">
                  Animation Room
                </div>
              )}
            </div>
            ...
            <aside className="min-h-0 overflow-hidden bg-[#f3eadb] p-4 text-slate-900 sm:p-5">
              <div>
                <p className="font-mono text-[9px] font-semibold tracking-[0.16em] text-[#8f3328]">
                  LEARN
                </p>
                <ul className="mt-3 space-y-2">
                  {lesson.objectives.map((objective) => (
                    <li key={objective} className="flex gap-2 text-xs leading-5 text-slate-700">
                      <span
                        className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#8f3328]"
                        aria-hidden="true"
                      />
                      <span>{objective}</span>
                    </li>
              ...
              <div className="mt-4 border-t border-[#8f3328]/15 pt-3">
                ...
                <p className="mt-2 text-xs font-semibold leading-6 text-[#8f3328]">
                  {lesson.takeaway}
                </p>
              </div>
            </aside>
        ...
        <footer className="shrink-0 pb-2.5 sm:pb-3">
          <div className="flex items-center justify-between gap-3">
            {previous ? (
              <Link
                to={previous.href}
                className="min-w-0 truncate rounded-xl border border-[#8f3328]/15 bg-white/65 px-3 py-2 text-xs font-semibold text-slate-800 hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8f3328]"
              >
                ← {previous.title}
              </Link>
            ) : (
            ...
            {next ? (
              <Link
                to={next.href}
                className="min-w-0 truncate rounded-xl bg-[#8f3328] px-4 py-2 text-xs font-semibold text-white hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8f3328]"
              >
                บทถัดไป: {next.title} →
              </Link>
            ) : (
              <Link
                to="/"
                className="rounded-xl bg-[#8f3328] px-4 py-2 text-xs font-semibold text-white hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#8f3328]"
              >
                กลับสู่ศูนย์การเรียนรู้ →
              </Link>
            )}
          </div>
        </footer>
...
export function LacPage() {
      ...
      <div className="min-h-screen bg-rac-surface font-['Mitr'] text-slate-900">
        ...
        <main className="min-h-[calc(100vh-60px)] px-4 py-20">
          <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <Link
              to="/"
              className="mt-6 inline-flex rounded-xl bg-rac-blue px-4 py-2 text-sm font-semibold text-white"
            >
              กลับหน้าแรก
            </Link>
          </div>
  ...
  if (slug === "knowledge") {
    content = (
      <main className="min-h-[calc(100vh-60px)] bg-rac-surface px-4 py-20 text-slate-900">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 max-w-3xl">
            <p className="text-xs font-semibold tracking-[0.18em] text-rac-lac">
              ความรู้เรื่องครั่ง
            </p>
            <h1 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">
              ภาพรวมความรู้เรื่องครั่ง
            </h1>
            <p className="mt-3 text-slate-600">
              เลือกหัวข้อเพื่อเข้าสู่ห้องเรียนรู้แบบ Interactive Lesson
            </p>
          </div>
          <LacKnowledgeCards />
        </div>
    ...
    ["what-is-lac", "life-cycle", "habitat", "host-plants", "lac-farming"].includes(slug)
  ) {
    ...
    content = (
      <main className="min-h-[calc(100vh-60px)] bg-rac-surface px-4 py-20 text-slate-900">
        <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8">
          <p className="text-sm font-semibold text-rac-lac">{page.title}</p>
          <h1 className="mt-2 text-3xl font-bold">หน้านี้อยู่ระหว่างพัฒนา</h1>
          <p className="mt-3 text-slate-600">
            องค์ความรู้ส่วนนี้จะเชื่อมเข้าสู่ LessonRoom ในขั้นถัดไป
          </p>
          <Link
            to="/lac/knowledge"
            className="mt-6 inline-flex rounded-xl bg-rac-lac px-4 py-2 text-sm font-semibold text-white"
          >
            กลับไปความรู้ทั้งหมด
          </Link>
        </div>
      </main>
    );
