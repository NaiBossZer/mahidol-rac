...
check("incorrect answer applies the configured penalty", bingoEngine.includes("incorrectPenalty: 5") && bingoEngine.includes("nextScore = Math.max(0, nextScore - difficultyRules.incorrectPenalty)"));
...
check("hook stops timer at game over", bingoHook.includes("if (state.isGameOver || !BINGO_DIFFICULTY_RULES[state.difficulty].hasTimeLimit) return;") && bingoHook.includes("return () => window.clearInterval(timer);"));
...
check("question modal copy matches configured score rules", mGuidePopup.includes("ระดับ") && mGuidePopup.includes("difficultyRules.label") && mGuidePopup.includes("ตอบถูก") && mGuidePopup.includes("difficultyRules.correctScore") && mGuidePopup.includes("ตอบผิด") && mGuidePopup.includes("difficultyRules.incorrectPenalty"));
